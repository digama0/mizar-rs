...
use crate::proof::{CoProof, ProofId, ProofKind, Step};
...
impl Reader {
  ...
  pub fn run_analyzer(&mut self, path: &MizPath, parser: Option<&mut Parser<'_>>) {
    ...
    if elab.g.cfg.mpt_json {
      elab.write_mpt.finish();
    }
...
impl Analyzer<'_> {
  ...
  pub fn push_prop(
    &mut self, label: Option<(Option<LabelId>, Rc<str>)>, prop: Formula, pf: ProofId,
  ) {
    ...
    self.r.push_prop(label, prop, pf);
  }
    ...
    match &mut it.kind {
      ...
      ast::ItemKind::Theorem { prop, just } => {
        ...
        if self.g.cfg.analyzer_full {
          ...
          if prop.label.is_some() {
            self.proof.with_mut(|p| p.set_local_scope(true));
          }
          let f = f.visit_cloned(&mut self.r.intern_const());
          let label = prop.label.as_ref().map(|l| (l.id, l.spelling.clone()));
          self.proof.with_mut(|p| p.set_local_scope(true));
          let pf = self.elab_justification_intro_reserved(label.as_ref(), &f, just, block);
          self.write_xml.on(|w| w.end_theorem());
          let pf = self.proof.with_mut(|p| {
            let pf = p.maybe_externalize(false, true, pf);
            p.step(proof::Step::Theorem);
            p.set_local_scope(false);
            pf
          });
          self.push_prop(label, f, pf)
        }
  ...
  fn elab_scheme(
    ...
    self.proof.with_mut(|p| {
      assert!(p.inner().ctx() == ProofId::C0);
      p.set_local_scope(true)
    });
    ...
    if self.g.cfg.analyzer_full {
      let pf = self.elab_proof_intro_reserved(None, &thesis, items, (pos, *end), block);
      self.write_xml.on(|w| w.end_scheme(*end));
      self.proof.with_mut(|p| {
        p.output(false, true, pf);
        p.set_local_scope(false);
        p.step(Step::Scheme)
      })
    }
    ...
    match &mut it.kind {
      ...
      ast::ItemKind::Reconsider { vars, ty, just } => {
        ...
        if let Some(conjs) = conjs {
          ...
          let pf = self.elab_justification(None, &f, just);
          self.r.proof.inner_mut(|p| p.read_reconsider(&self.r.lc.fixed_var, pf));
      ...
      ast::ItemKind::Consider { vars, conds, just } => {
        ...
        if self.g.cfg.analyzer_full {
          ...
          let pf = self.r.proof.with_mut(|p| p.def_const(&self.r.lc.fixed_var, pf));
          let mut cur = 0;
          for (_, label, mut f) in to_push {
            let size = f.conjuncts().len() as u32;
            f.visit(&mut self.intern_const());
            let pf = self.proof.inner_mut(|p| p.and_elim(pf, cur, size));
            cur += size;
            self.push_prop(label, f, pf);
          }
        }
      ...
      ast::ItemKind::Statement(stmt) =>
        if self.g.cfg.analyzer_full {
          self.elab_stmt(it.pos, stmt);
        },
      _ => unreachable!("unexpected item: {it:?}"),
    }
  }

  fn elab_stmt(&mut self, pos: Position, stmt: &mut ast::Statement) -> Formula {
    ...
    match stmt {
      ast::Statement::Proposition { prop, just } => {
        ...
        let label = prop.label.as_ref().map(|l| (l.id, l.spelling.clone()));
        let pf = self.elab_justification_intro_reserved(label.as_ref(), &f, just, block);
        self.push_prop(label, f.clone(), pf);
        f
      }
        ...
        if let Formula::Pred { nr, ref args } = f {
          if let (nr, [lhs, rhs]) = Formula::adjust_pred(nr, args, Some(&self.g.constrs)) {
            if self.g.reqs.equals_to() == Some(nr) {
              let old = self.proof.with_mut(|p| p.set_local_scope(true));
              self.write_xml.on(|w| {
                let label = prop
                  .label
                  .as_ref()
              ...
              for ast::IterStep { rhs, just, .. } in steps {
                ...
                let pf2 = self.elab_justification(None, &self.g.reqs.mk_eq(mid, rhs.clone()), just);
                pf = self.proof.with_mut(|p| p.eq_trans(pf, pf2));
                mid = rhs;
              }
              self.write_xml.on(|w| {
                w.end_iter_step();
              ...
              pf = self.proof.with_mut(|p| {
                let pf = p.maybe_externalize(old, false, pf);
                p.set_local_scope(old);
                pf
              });
      ...
      ast::Statement::Now { end, label, items } => {
        ...
        let (f, pf) = self.scope(label.is_some(), false, true, |this| {
          ReconstructThesis { stack: vec![ProofStep::Break(true)] }.elab_proof(this, items, *end)
        });
        self.write_xml.on(|w| w.end_now());
        self.push_prop(label, f.clone(), pf);
        f
      }
    }
  ...
  fn elab_proof_intro_reserved(
    ...
    self.scope(label.is_some(), false, false, |this| {
      ...
      this.r.proof.with_mut(|p| {
        dbg!(this.r.lc.fixed_var.len());
        let stmt = p.tr_formula(&this.r.lc, dbg!(thesis));
        let inner = p.inner();
        inner.coproof_stack.push(proof::CoProof::Justify { stmt });
        inner.thesis = stmt;
      });
      this.thesis = Some(Box::new(thesis.clone()));
      reset.intro(this);
      let pf = WithThesis.elab_proof(this, items, end);
  ...
  fn elab_fixed_vars(&mut self, ast::BinderGroup { vars, ty }: &mut ast::BinderGroup) {
    ...
    if let Some(ty) = ty {
      let ty = self.elab_intern_type_no_reserve(ty);
      let pty = self.r.proof.with_mut(|p| p.tr_type(&self.r.lc, &ty));
      for var in &*vars {
        self.r.proof.inner_mut(|inner| inner.push_fixed_var(pty));
        let id = self.intern_id(&var.spelling);
      ...
      self.r.proof.with_mut(|p| {
        let ty = p.tr_type(&self.r.lc, &ty);
        p.inner().push_fixed_var(ty)
      });
  ...
  fn elab_proposition_forall_reserved(
    ...
    if quotable && self.g.cfg.analyzer_full {
      let pf = self.r.proof.with_mut(|p| {
        let stmt = p.tr_formula(&self.r.lc, &f);
        p.inner().assume(stmt)
      });
      self.push_prop(prop.label.as_ref().map(|l| (l.id, l.spelling.clone())), f.clone(), pf);
    }
  ...
  fn elab_canceled(&mut self, pos: Position, kind: CancelKind, n: u32) {
    ...
    self.write_xml.on(|w| w.canceled(kind, n))
  }

  fn elab_functor_term(&mut self, pos: Position, fmt: FormatFunc, args: &[ast::Term]) -> Term {
    ...
    for pat in self.notations[PKC::Func].iter().rev() {
      if pat.fmt == fmt {
        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
          ...
          self.write_mpt.on(|w| w.write(pos, fmt, nr));
  ...
  fn elab_pred(
    &mut self, pos: Position, fmt: FormatPred, args: Vec<TermQua>, positive: bool,
  ) -> Formula {
    let fmt = self.formats[&Format::Pred(fmt)];
    for pat in self.r.notations[PKC::Pred].iter().rev() {
      if pat.fmt == fmt {
        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
          ...
          args.iter_mut().for_each(|t| t.strip_qua_mut());
          return Formula::Pred { nr, args }.maybe_neg(pat.pos == positive)
        }
      }
    }
    panic!("type error")
  ...
  fn elab_term_qua(&mut self, tm: &ast::Term) -> TermQua {
    ...
    let res = (|| match *tm {
      ...
      ast::Term::Infix { pos, sym, left, ref args, .. } => {
        let fmt = FormatFunc::Func { sym, left, right: args.len() as u8 - left };
        self.elab_functor_term(pos, fmt, args)
      }
      ast::Term::Bracket { pos, lsym, rsym, ref args, .. } => {
        let fmt = FormatFunc::Bracket { lsym, rsym, args: args.len() as u8 };
        self.elab_functor_term(pos, fmt, args)
      }
      ...
      ast::Term::Aggregate { pos, sym, ref args, .. } => {
        let args = self.elab_terms_qua(args);
      ...
      ast::Term::SubAggr { pos, sym, ref arg, .. } => {
      ...
      ast::Term::Selector { pos, sym, ref arg, .. } => {
        let arg = self.elab_term_qua(arg);
        ...
        for pat in self.r.notations[PKC::Attr].iter().rev() {
          if pat.fmt == fmt {
            if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
              ...
              return Formula::Attr { nr, args: args.to_owned().into() }
                .maybe_neg(pat.pos == positive)
            }
          }
        }
        panic!("type error")
  ...
  /// AnalyzeAttribute
  fn elab_attr(&mut self, attr: &ast::Attr, mut positive: bool, ty: &mut Type) -> Attr {
    match attr {
      ast::Attr::Non { attr, .. } => self.elab_attr(attr, !positive, ty),
      &ast::Attr::Attr { pos, sym, ref args, .. } => {
          ...
          if pat.fmt == fmt {
            if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
              ...
              *ty = self.r.lc.bound_var.0.pop().unwrap().1;
              positive = pat.pos == positive;
              assert!(matches!(ty.attrs.0, Attrs::Consistent(_)));
              let out = Attr { nr, pos: positive, args };
              // vprintln!("elab_attr {attr:?} <- {ty:?}\n  -> {out:?}");
              return out
            }
          }
  ...
  fn elab_type(&mut self, ty: &ast::Type) -> Type {
    ...
    let res = (|| match ty {
      &ast::Type::Mode { pos, sym, ref args, .. } => {
        let args = self.elab_terms_qua(args);
      ...
      &ast::Type::Struct { pos, sym, ref args, .. } => {
        let args = self.elab_terms_qua(args);
  ...
  fn elab_formula(&mut self, f: &ast::Formula, positive: bool) -> Formula {
    ...
    let res = match f {
      ...
      ast::Formula::Pred(pred) => {
        ...
        self.elab_pred(pred.pos, fmt, args.into(), pred.positive == positive)
      }
      ast::Formula::ChainPred { first, rest, .. } => {
        let ast::Pred { mut pos, mut sym, mut left, ref args, .. } = **first;
        ...
        Formula::mk_and_with(|conjs| {
          for rhs in rest {
            ...
            self.elab_pred(pos, fmt, args, rhs.positive).append_conjuncts_to(conjs);
            mid.extend(rhs.right.iter().map(|t| self.elab_term_qua(t)));
            (pos, args, sym, left) = (rhs.pos, mid, rhs.sym, right);
          }
          let right = args.len() as u8 - left;
          let fmt = FormatPred { sym, left, right };
          self.elab_pred(pos, fmt, args, first.positive).append_conjuncts_to(conjs);
        })
        .maybe_neg(positive)
      }
  ...
  fn try_unfold(
    &self, up: bool, i: DefiniensId, def: &Definiens, pos: bool, kind: ConstrKind, args: &[Term],
    pff: ProofId,
  ) -> Option<(bool, Formula, ProofId)> {
    ...
    let (mut pos2, mut f2) = if value.cases.is_empty() {
  ...
  fn whnf(
    &mut self, up: bool, mut atomic: usize, f: &mut (bool, Box<Formula>, ProofId),
    ...
    'start: loop {
      ...
      let (kind, args) = match &mut *f.1 {
        Formula::Neg { f: f2 } => {
          *f = (!f.0, std::mem::take(f2), self.proof.inner_mut(|inner| inner.neg(f.2)));
          continue 'start
        }
      ...
      for (i, def) in self.definitions.enum_iter().rev() {
        if let Some((pos2, f2, pf2)) = self.try_unfold(up, i, def, f.0, kind, args, f.2) {
          f.0 = pos2;
          *f.1 = f2;
          f.2 = pf2;
  ...
  fn inst_forall(
    &mut self, term: &Term, widenable: bool, up: bool, f: &mut (bool, Box<Formula>, ProofId),
  ...
  fn forall_telescope(
    &mut self, start: usize, widenable: bool, up: bool, f: &mut (bool, Box<Formula>, ProofId),
    mut expansions: Option<&mut BTreeMap<DefiniensId, u32>>,
  ) {
  ...
  fn and_telescope(
    ...
    'ok: loop {
      ...
      let mut f2 = (true, Box::new(iter2.next().unwrap_or(Formula::True)), ProofId::INVALID);
      loop {
  ...
  fn elab_assume(&mut self, conds: &mut [ast::Proposition]) -> Vec<Formula> {
    let mut conjs = vec![];
    ...
    self.write_xml.on(|w| w.end_assume());
    conjs
  }
}
...
impl ReserveBlock {
  fn intro(self, elab: &mut Analyzer<'_>) {
    if !self.0.is_empty() {
      ...
      let mut stmt = elab.r.proof.inner_mut(|inner| inner.thesis);
      for &nr in &self.0 {
        ...
        stmt = elab.r.proof.inner_mut(|inner| {
          let ProofKind::FForAll { ctx2, scope } = inner[stmt] else { unreachable!() };
          inner.intro(ctx2);
          inner.coproof_stack.push(CoProof::Let);
          scope
        });
        thesis = scope;
        inst.depth += 1;
      ...
      elab.r.proof.inner_mut(|inner| inner.thesis = stmt);
...
trait ReadProof {
  ...
  fn super_elab_item(
    ...
    match &mut it.kind {
      ...
      ast::ItemKind::Assume(asm) => {
        let conjs = elab.elab_assume(asm.conds());
        self.assume(elab, conjs, true)
      }
      ...
      ast::ItemKind::Take(its) =>
        for ast::TakeDecl { var, term } in its {
          ...
          if let Some(var) = var {
            ...
            elab.write_xml.on(|w| w.take(&elab.r.lc, &term));
            self.take(elab, term)
          }
        },
      ast::ItemKind::Thus(stmt) =>
        if elab.g.cfg.analyzer_full {
          elab.write_xml.on(|w| w.start_thus());
          let f = elab.elab_stmt(it.pos, stmt);
          elab.write_xml.on(|w| w.end_thus());
  ...
  fn elab_item(&mut self, elab: &mut Analyzer, item: &mut ast::Item, block_end: Position) -> bool {
    self.super_elab_item(elab, item, block_end)
  }
  ...
  fn elab_proof(
    ...
    for item in items {
      if !self.elab_item(elab, item, end) {
        break
      }
    }
    elab.write_xml.on(|w| w.end_pos(end));
...
impl ReadProof for WithThesis {
  ...
  fn intro(&mut self, elab: &mut Analyzer, start: usize, _: u32) {
    let t = elab.proof.inner_mut(|inner| inner.thesis);
    let mut thesis = (true, elab.thesis.take().unwrap(), t);
  ...
  fn take(&mut self, elab: &mut Analyzer, term: Term) {
    let mut thesis = (false, elab.thesis.take().unwrap(), ProofId::INVALID);
    let mut expansions = Default::default();
    let eref = elab.write_xml.on(|_| Some(&mut expansions));
  ...
  fn new_case(
    ...
    let mut err = None;
    let args = 'next: loop {
      let mut thesis = (false, Box::new(iter.next().unwrap_or(Formula::True)), ProofId::INVALID);
      loop {
  ...
  fn end_cases(
    &mut self, elab: &mut Analyzer, (mut case, expansions): Self::CaseIter, end: Position,
  ) {
  ...
  fn end_block(&mut self, elab: &mut Analyzer, end: Position) -> ProofId {
    let f = elab.thesis.as_deref().unwrap();
    if !matches!(f, Formula::True) {
      eprintln!(
    ...
    elab.proof.with_mut(|p| p.finish_with_thesis_block())
  }
}
...
impl ReconstructThesis {
  fn reconstruct(
    ...
    let mut rec = Reconstruction { pos, conjs, pf: ProofId::INVALID };
    loop {
      match self.stack.pop().unwrap() {
        ...
        ProofStep::Break(pos2) => {
          assert_eq!(pos, pos2);
          return (Formula::mk_and(std::mem::take(rec.as_pos(pos))), rec.pf)
        }
      }
    }
  }
...
impl ReadProof for ReconstructThesis {
  ...
  fn end_case(&mut self, elab: &mut Analyzer, _: &mut (), (f, pf): (Formula, ProofId)) {
    self.assume(elab, f.mk_neg().into_conjuncts(), false);
  }

  fn end_cases(&mut self, elab: &mut Analyzer, _: (), end: Position) {
    let (f, pf) = self.reconstruct(elab, false, None);
    let f = f.mk_neg();
    elab.write_xml.on(|w| {
      w.end_pos(end);
      w.write_block_thesis(&elab.r.lc, std::iter::empty(), &f)
  ...
  fn end_suppose(
    &mut self, elab: &mut Analyzer, recv: &mut Self::SupposeRecv, (f, pf): (Formula, ProofId),
  ) {
  ...
  fn end_block(&mut self, elab: &mut Analyzer, _: Position) -> (Formula, ProofId) {
    ...
    elab.write_xml.on(|w| {
      ...
      w.write_block_thesis(&elab.r.lc, theses_rev.iter().rev(), &f.0);
...
impl BlockReader {
  ...
  fn after_scope(self, elab: &mut Analyzer) {
    ...
    for (pos, def) in self.defs {
      if let Some(PendingDef { kind, df, label, thm, pf }) = def {
  ...
  fn elab_func_def(
    ...
    if it.redef {
      ...
      let PatternKind::Func(nr) = pat.kind else { unreachable!() };
      elab.write_mpt.on(|w| w.write(pos, fmt, nr));
    ...
    if let Some(mut value) = value {
      ...
      let pf = ProofId::INVALID;
      self.defs.push((loc, Some(PendingDef { kind: ConstrKind::Func(n), df, label, thm, pf })));
    }
  ...
  fn elab_pred_def(
    ...
    let mut properties = PropertiesBuilder::new(&visible);
    let (redefines, superfluous, positive);
    if it.redef {
      let pos = pat.pos;
      let args: Box<[_]> = pat.args.iter().map(|v| Term::Const(v.var())).collect();
      let pat = elab.r.notations[PKC::Pred].iter().rev().find(|pat| {
        pat.fmt == fmt
      ...
      (redefines, superfluous, positive) =
        (Some(nr), (self.primary.len() - c.primary.len()) as u8, pat.pos);
      properties.props = c.properties.offset(superfluous)
    } else {
      (redefines, superfluous, positive) = (None, 0, true)
    }
    let value = def.as_deref_mut().map(|def| elab.elab_def_value(&mut def.kind, positive));
    if let Some(value) = &value {
    ...
    if let Some(value) = value {
      ...
      let pf = ProofId::INVALID;
      self.defs.push((loc, Some(PendingDef { kind: ConstrKind::Pred(n), df, label, thm, pf })));
    }
    elab.push_mk_pattern(true, PKC::Pred, PatternKind::Pred(n), fmt, primary, visible, positive);
    elab.write_xml.on(|w| w.end_def())
  }
    ...
    match kind {
      ...
      ast::DefModeKind::Standard { spec, def } => {
        ...
        if let Some(value) = value {
          ...
          let label = def.as_ref().unwrap().label.as_ref().map(|l| (l.id, l.spelling.clone()));
          let pf = ProofId::INVALID;
          self.defs.push((loc, Some(PendingDef { kind: ConstrKind::Mode(n), df, label, thm, pf })));
        }
  ...
  fn elab_attr_def(
    ...
    let (redefines, superfluous, pos) = if it.redef {
      let pos = pat.pos;
      let args: Box<[_]> = pat.args.iter().map(|v| Term::Const(v.var())).collect();
      let pat = elab.r.notations[PKC::Attr].iter().rev().find(|pat| {
        pat.fmt == fmt
    ...
    if let Some(value) = value {
      ...
      let label = def.as_ref().unwrap().label.as_ref().map(|l| (l.id, l.spelling.clone()));
      let pf = ProofId::INVALID;
      self.defs.push((loc, Some(PendingDef { kind: ConstrKind::Attr(n), df, label, thm, pf })));
    }
        ...
        for pat in elab.notations[PKC::Sel].iter().rev() {
          if pat.fmt == fmt {
            if let Some(subst) = pat.check_types(&elab.g, &elab.lc, std::slice::from_ref(&arg)) {
              let PatternKind::Sel(nr) = pat.kind else { unreachable!() };
              elab.write_mpt.on(|w| w.write(v.pos, fmt, nr));
  ...
  fn elab_identify_pattern_func(
    &self, elab: &mut Analyzer, pat: &mut ast::PatternFunc,
  ) -> PatternFuncResult {
    let pos = pat.pos();
    let fmt = elab.formats[&Format::Func(pat.to_format())];
    let visible: Box<[_]> =
    ...
    let args: Box<[_]> = pat.args().iter().map(|v| Term::Const(v.var())).collect();
    for pat in elab.r.notations[PKC::Func].iter().rev() {
      if pat.fmt == fmt {
  ...
  fn elab_pred_notation(
    ...
    let pat = pat.expect("type error");
    let PatternKind::Pred(nr) = pat.kind else { unreachable!() };
    elab.write_mpt.on(|w| w.write(orig.pos, fmt_orig, nr));
    let c = &elab.g.constrs.predicate[nr];
  ...
  fn elab_func_notation(
    ...
    let args: Box<[_]> = orig.args().iter().map(|v| Term::Const(v.var())).collect();
    let pat = elab.r.notations[PKC::Func].iter().rev().find(|pat| {
      pat.fmt == fmt_orig
        && !matches!(pat.kind, PatternKind::Func(nr)
            if elab.g.constrs.functor[nr].redefines.is_some())
        && matches!(pat.check_types(&elab.g, &elab.lc, &args),
  ...
  fn elab_mode_notation(
    ...
    let args: Box<[_]> = orig.args.iter().map(|v| Term::Const(v.var())).collect();
    let pat = elab.r.notations[PKC::Mode].iter().rev().find(|pat| {
      pat.fmt == fmt_orig
        && !matches!(pat.kind, PatternKind::Mode(nr)
            if elab.g.constrs.mode[nr].redefines.is_some())
        && matches!(pat.check_types(&elab.g, &elab.lc, &args),
...
impl ReadProof for BlockReader {
  ...
  fn elab_item(&mut self, elab: &mut Analyzer, it: &mut ast::Item, block_end: Position) -> bool {
    match &it.kind {
